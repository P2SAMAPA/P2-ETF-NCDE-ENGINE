...
class VectorField(nn.Module):
    ...
    Fix vs original: the intermediate layer dimensions were hardcoded to
    cfg.VECTOR_FIELD_DIM only for the first layer; subsequent layers fell
    back to the wrong dimension when n_layers >= 3. Now all hidden layers
    correctly use VECTOR_FIELD_DIM.

    Added LayerNorm after each Tanh to stabilise the hidden trajectory —
    important when macro features have very different scales from price returns.
    """
    ...
    def __init__(self, input_dim: int, hidden_dim: int, n_layers: int, dropout: float):
        ...
        for i in range(n_layers):
            ...
            layer_out = out_dim if is_last else vf_dim
            layers.append(nn.Linear(in_dim, layer_out))
            if not is_last:
                layers.append(nn.Tanh())
                layers.append(nn.LayerNorm(vf_dim))  # stabilise path dynamics
...
class NCDEModel(nn.Module):
    ...
    def __init__(self, input_dim: int, hidden_dim: int, n_layers: int, dropout: float):
        ...
        # Added a tanh to keep h0 in a reasonable range before ODE integration
    ...
    def forward(self, X_path: torchcde.CubicSpline) -> torch.Tensor:
        ...
        # h_T shape: (batch, ODE_STEPS+1, hidden_dim) — take terminal state
        return h_T[:, -1, :]
...
class ReadoutHead(nn.Module):
    ...
    Changes vs original:
      - Three layers instead of two (hidden → readout_dim → readout_dim//2 → out)
        was creating a severe bottleneck when readout_dim == hidden_dim.
      - Now: hidden → readout_dim → readout_dim → out (no bottleneck layer).
      - BatchNorm on h(T) before readout to normalise terminal state distribution.
      - SiLU activations (smoother gradient than ReLU for financial outputs).
    """

    def __init__(self, hidden_dim: int, readout_dim: int, n_assets: int, dropout: float):
        super().__init__()

        self.norm = nn.LayerNorm(hidden_dim)  # normalise terminal hidden state
        ...
        self.net = nn.Sequential(
            ...
            nn.Linear(readout_dim, n_assets * 2),  # mu + log_sigma per asset
        )
